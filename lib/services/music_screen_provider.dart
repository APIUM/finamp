...
/// Total item count for [request], ignoring pagination. The CarPlay letter
/// layer uses this to decide whether a view needs the A-Z drill-down.
@riverpod
...
Future<QueryResult_BaseItemDto> _fetchMusicScreenPageOnline(
  ...
  final letterFilter = request.sortConfig.filters.firstWhereOrNull(
    (x) => x.type == ItemFilterType.startsWithCharacter,
  );
  ...
  return jellyfinApiHelper.getItemsWithTotalRecordCount(
    ...
    // "#" follows jellyfin-web and maps to NameLessThan=A. Names starting
    // past "z" (accented or non-Latin scripts) match no online bucket.
