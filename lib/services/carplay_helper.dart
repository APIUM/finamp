...
const _enableOnlineTracksLetterDrilldown = false;
...
class CarPlayHelper {
  ...
  /// Whether [tab] can show the A-Z letter drill-down. See
  /// [_enableOnlineTracksLetterDrilldown] for why online Tracks is excluded.
  bool _letterLayerSupported(ContentType tab) =>
      FinampSettingsHelper.finampSettings.isOffline || tab != ContentType.tracks || _enableOnlineTracksLetterDrilldown;
    ...
    if (!groupByLetter) {
      final flatItems = [
        for (final (index, item) in items.indexed)
          itemCache.putIfAbsent(item.id.raw, () => itemBuilder(item, index)),
      ];
  ...
  /// Chooses between a flat progressive list and the A-Z letter picker for a
  ...
  Future<void> _pushProgressiveListTemplate({
    ...
    // Repaints the on-screen template as pages arrive, driving new page
    // requests and the first push. A stale update on a popped template is a
    // native no-op, but cancellation is checked so the driver stops promptly
    // on back-out.
    driver = providerRef.listen<PagingState<int, FinampDisplayableOrPlayable>>(provider, fireImmediately: true, (
      _,
      next,
      ...
      final sections = _buildProgressiveSections(items, maxSections, itemBuilder, itemCache,
          leadingItem: leadingItem, groupByLetter: groupByLetter);
      ...
      if (template == null) {
        ...
        unawaited(FlutterCarplay.push(template: pushTemplate).then((_) {
  ...
  Future<void> showBrowsableListTemplate({required ContentType tabType, BaseItemDto? genreFilter}) async {
    ...
    try {
      ...
      await _showLibraryTemplate(
        ...
        itemBuilderFor: (_) => (item, index) => CPListItem(
  ...
  Future<void> showTracksTemplate() async {
    ...
    try {
      await _showLibraryTemplate(
        ...
        itemBuilderFor: (request) => (item, index) => CPListItem(
          text: item.name ?? GlobalSnackbar.requireL10n.unknownName,
          detailText: item.artists?.join(", ") ?? item.albumArtist,
          image: _getCarPlayImageUri(item),
          onPress: (complete, self) async {
